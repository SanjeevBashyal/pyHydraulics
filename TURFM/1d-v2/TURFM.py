...
DEFAULT_MODEL_FOLDER = "BUR-BUR-TEF-TEFENNI1_Rev_V1"
...
def _build_model_config(model_folder: str) -> ModelConfig:
    ...
    structure_csv = _select_optional_single_file(
        roleve_dir / "structure_dim",
        "*.csv",
    )
    return ModelConfig(
...
def _build_argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--structure-csv",
        help="Optional structure CSV for single-model mode.",
    )
    parser.add_argument(
        "--main-structure-csv",
        help="Optional structure CSV for the main-stem junction model.",
    )
    parser.add_argument(
        "--tributary-structure-csv",
        help="Optional structure CSV for the tributary junction model.",
    )
    parser.add_argument(
        "--no-structures",
        action="store_true",
        help="Disable structure loading even when a structure CSV is discovered.",
    )
    parser.add_argument(
...
def main(argv: list[str] | None = None):
    ...
    if junction_mode:
        ...
        if not args.no_structures:
            main_structure_csv = (
                Path(args.main_structure_csv)
                if args.main_structure_csv
                else main_model.structure_csv
            )
            tributary_structure_csv = (
                Path(args.tributary_structure_csv)
                if args.tributary_structure_csv
                else tributary_model.structure_csv
            )
        combined_name = (
            f"{main_model.model_folder}__{tributary_model.model_folder}"
        )
        ...
        logger.info("Main structure CSV: %s", main_structure_csv)
        logger.info("Tributary structure CSV: %s", tributary_structure_csv)
        ...
        logger.info("Structure CSV: %s", structure_csv)
