...
class HECRAS:
    ...
    def compute_project(
        ...
        if geometry_hdf_prepared and geometry_context is not None:
            ...
            self._patch_geometry_hdf_bank_lines(
                geometry_hdf_path,
                raw_bank_lines=geometry_context.get("hdf_bank_lines_raw"),
                projected_bank_lines=geometry_context.get("hdf_bank_lines_projected"),
                left_bank_line=geometry_context["left_bank_line"],
                right_bank_line=geometry_context["right_bank_line"],
                centerline_line=geometry_context.get("centerline_line"),
            )
            self._patch_geometry_hdf_cross_sections(
                geometry_hdf_path,
                geometry_context["sections"],
            )
            if plan_hdf_path.exists():
                self._patch_geometry_hdf_bank_lines(
                    plan_hdf_path,
                    raw_bank_lines=geometry_context.get("hdf_bank_lines_raw"),
                    projected_bank_lines=geometry_context.get(
    ...
    def _prepare_corrected_geometry_hdf(
        ...
        project_stem: str,
        geometry_context: dict[str, Any],
        dummy_plan_number: str = "99",
    ) -> None:
        if geometry_context.get("junction"):
            logger.info(
                "Skipping dummy geometry-HDF bank-line correction for junction models."
            )
            return
        if "left_bank_line" not in geometry_context or "right_bank_line" not in geometry_context:
            logger.info("Skipping geometry-HDF bank-line correction; bank lines missing.")
            return
        ...
        if geometry_hdf_path.exists():
            ...
            logger.info(
                "Created and patched geometry HDF bank lines using p01 pre-run: %s",
                geometry_hdf_path,
            )
        else:
            logger.warning(
    ...
    def _create_dummy_compute_plan(
        self,
        project_folder: Path,
        project_stem: str,
        dummy_plan_number: str,
    ) -> None:
        dummy_plan_number = self._normalize_plan_number(dummy_plan_number)
        source_flow = project_folder / f"{project_stem}.f01"
        source_plan = project_folder / f"{project_stem}.p01"
        dummy_flow = project_folder / f"{project_stem}.f{dummy_plan_number}"
        dummy_plan = project_folder / f"{project_stem}.p{dummy_plan_number}"
        project_path = project_folder / f"{project_stem}.prj"

        if not source_flow.exists() or not source_plan.exists() or not project_path.exists():
            raise FileNotFoundError(
                "Cannot create dummy geometry-HDF plan; required p01/f01/prj "
                "files are missing."
            )

        self._write_dummy_steady_flow_file(
            source_flow=source_flow,
            dummy_flow=dummy_flow,
            flow_value=15.0,
        )
        plan_text = source_plan.read_text(encoding="utf-8", errors="ignore")
        plan_text = plan_text.replace("Flow File=f01", f"Flow File=f{dummy_plan_number}")
        plan_text = re.sub(
            r"^Plan Title=.*$",
            f"Plan Title=Dummy Geometry HDF p{dummy_plan_number}",
            plan_text,
            count=1,
            flags=re.MULTILINE,
        )
        plan_text = re.sub(
            r"^Short Identifier=.*$",
            f"Short Identifier={'Dummy Geometry HDF'[:64]:<64}",
            plan_text,
            count=1,
            flags=re.MULTILINE,
        )
        dummy_plan.write_text(plan_text, encoding="utf-8")

        project_lines = project_path.read_text(
            encoding="utf-8",
            errors="ignore",
        ...
        for line in project_lines:
            if line.startswith("Current Plan="):
                updated_lines.append(f"Current Plan=p{dummy_plan_number}\n")
                continue
            updated_lines.append(line)
            if line.startswith("Plan File=p01") and not inserted:
                updated_lines.append(f"Flow File=f{dummy_plan_number}\n")
                updated_lines.append(f"Plan File=p{dummy_plan_number}\n")
                inserted = True
        if not inserted:
            updated_lines.extend(
                [
                    f"Flow File=f{dummy_plan_number}\n",
                    f"Plan File=p{dummy_plan_number}\n",
                ]
            )
        project_path.write_text("".join(updated_lines), encoding="utf-8")

    @staticmethod
    def _write_dummy_steady_flow_file(
        source_flow: Path,
        dummy_flow: Path,
        flow_value: float = 15.0,
    ) -> None:
        source_text = source_flow.read_text(encoding="utf-8", errors="ignore")
        river_line = re.search(r"^River Rch & RM=(.+)$", source_text, flags=re.MULTILINE)
        boundary_line = re.search(
            r"^Boundary for River Rch & Prof#=(.+)$",
            source_text,
            flags=re.MULTILINE,
        )
        up_slope = re.search(r"^Up Slope=(.+)$", source_text, flags=re.MULTILINE)
        dn_slope = re.search(r"^Dn Slope=(.+)$", source_text, flags=re.MULTILINE)
        if not river_line or not boundary_line:
            raise ValueError(f"Could not parse required flow metadata from {source_flow}")

        content = [
            "Flow Title=Dummy_Geometry_HDF\n",
            "Program Version=6.60\n",
            "Number of Profiles= 1 \n",
            "Profile Names=Dummy\n",
            f"River Rch & RM={river_line.group(1).rstrip()}\n",
            f"{HECRAS._format_number(flow_value, decimals=3):>8}\n",
            f"Boundary for River Rch & Prof#={boundary_line.group(1).rstrip()}\n",
            "Up Type= 3 \n",
            f"Up Slope={(up_slope.group(1).strip() if up_slope else '0.001')}\n",
            "Dn Type= 3 \n",
            f"Dn Slope={(dn_slope.group(1).strip() if dn_slope else '0.001')}\n",
        ...
        dummy_flow.write_text("".join(content), encoding="utf-8")

    @staticmethod
    def _delete_dummy_compute_plan_outputs(
        project_folder: Path,
        project_stem: str,
        dummy_plan_number: str,
    ) -> None:
        dummy_plan_number = HECRAS._normalize_plan_number(dummy_plan_number)
        suffixes = [
            f".p{dummy_plan_number}",
            f".f{dummy_plan_number}",
            f".p{dummy_plan_number}.hdf",
            f".p{dummy_plan_number}.tmp.hdf",
            f".b{dummy_plan_number}",
            f".c{dummy_plan_number}",
            f".O{dummy_plan_number}",
            f".o{dummy_plan_number}",
            f".r{dummy_plan_number}",
            f".comp_msgs{dummy_plan_number}.txt",
        ]
        for suffix in suffixes:
            ...
            if path.exists():
                try:
                    ...
                    logger.warning("Could not delete dummy output %s: %s", path, exc)
    ...
    def _prepare_geometry_context(
        ...
        friction_slopes = self._estimate_terminal_friction_slopes(sections)
        return {
            "river": river,
            "reach": reach,
            "sections": sections,
            ...
            "centerline_line": centerline_line,
            "friction_slope": friction_slopes["downstream"],
            "friction_slopes": friction_slopes,
    ...
    def _prepare_junction_geometry_context(
        ...
        tributary_context = {
            **tributary_context,
            ...
            "centerline_line": LineString(tributary_centerline_points),
            "friction_slope": self._estimate_terminal_friction_slopes(
                tributary_sections
            )["downstream"],
            "friction_slopes": self._estimate_terminal_friction_slopes(
                tributary_sections
            ),
    ...
    def _build_reach_context_from_existing_context(
        ...
        return {
            ...
            "centerline_measures": centerline_measures,
            "centerline_line": LineString(centerline_points),
            "friction_slope": self._estimate_terminal_friction_slopes(subset)[
                "downstream"
            ],
            "friction_slopes": self._estimate_terminal_friction_slopes(subset),
        }
    ...
    def _load_bank_line_features_for_hdf(
        ...
        # HEC-RAS stores imported bank-line GIS features in reverse feature order
        # in the geometry HDF. Preserve that convention so generated HDF layers
        # match models opened/saved through the HEC-RAS UI.
    ...
    def _write_model_files(
        ...
        if geometry_context.get("junction"):
            ...
            friction_slopes = geometry_context["lower_reach"].get(
                "friction_slopes",
                {"upstream": friction_slope, "downstream": friction_slope},
            )
            ...
            friction_slopes = geometry_context.get(
                "friction_slopes",
                {"upstream": friction_slope, "downstream": friction_slope},
            )
            ...
            self._write_steady_flow_file(
                ...
                upstream_friction_slope=friction_slopes["upstream"],
                downstream_friction_slope=friction_slopes["downstream"],
                flow_profiles=flow_profiles,
            )
    ...
    def _build_structures(
        ...
        sections: list[SectionData],
        centerline_line: LineString,
        left_bank_line: Optional[LineString] = None,
        right_bank_line: Optional[LineString] = None,
    ) -> list[StructureData]:
        ...
        for idx, row in structure_df.iterrows():
            ...
            if structure_type == "box":
                ...
                if "Diameter" not in structure_df.columns and "diameter" not in structure_df.columns:
                    raise ValueError(
                        "Circular structure table must include a Diameter column."
                    )
                culvert_shape_code = 1
                culvert_shape_name = "CIRCULAR"
                culvert_rise = self._row_float_any(row, ("Diameter", "diameter"))
            ...
            # Older circular-culvert TOML files used centerline_half_length to
            # force channel-centered placement. Circular culverts now use the
            # same bank-offset/CSV override station logic as box culverts.
            if opening_station_method == "centerline_half_length":
                opening_station_method = "bank_offset"
            if opening_station_method not in {"bank_offset", "csv_override"}:
                raise ValueError(
                ...
                StructureData(
                    ...
                    htab_hwmax=self._row_float(
                        ...
                        default=max(
                            upstream_deck_elevations
                            + downstream_deck_elevations
                            + [upstream_low_chord, downstream_low_chord]
                        )
                        + 0.09,
                    ),
    ...
    @staticmethod
    def _structure_face_chainage_length(
        upstream_points: list[Point],
        downstream_points: list[Point],
        fallback_length: float,
    ) -> float:
        upstream_width = float(upstream_points[0].distance(upstream_points[1]))
        downstream_width = float(downstream_points[0].distance(downstream_points[1]))
        surveyed_face_length = 0.5 * (upstream_width + downstream_width)
        return max(float(fallback_length), surveyed_face_length)

    @staticmethod
    def _structure_face_sections_from_center(
        sections: list[SectionData],
        river_station: float,
        downstream_measure: float,
        fallback_upstream_section: SectionData,
        fallback_downstream_section: SectionData,
        face_chainage_length: float,
        left_bank_line: Optional[LineString] = None,
        right_bank_line: Optional[LineString] = None,
    ) -> tuple[SectionData, SectionData]:
        half_length = 0.5 * max(float(face_chainage_length), 0.0)
        upstream_measure = float(downstream_measure) - (
            float(river_station) + half_length
        )
        downstream_measure_at_face = float(downstream_measure) - (
            float(river_station) - half_length
        )
        upstream_face = HECRAS._interpolate_structure_face_section(
            sections=sections,
            target_measure=upstream_measure,
            fallback_section=fallback_upstream_section,
            left_bank_line=left_bank_line,
            right_bank_line=right_bank_line,
        )
        downstream_face = HECRAS._interpolate_structure_face_section(
            sections=sections,
            target_measure=downstream_measure_at_face,
            fallback_section=fallback_downstream_section,
            left_bank_line=left_bank_line,
            right_bank_line=right_bank_line,
        )
        return upstream_face, downstream_face

    @staticmethod
    def _interpolate_structure_face_section(
        sections: list[SectionData],
        target_measure: float,
        fallback_section: SectionData,
        left_bank_line: Optional[LineString] = None,
        right_bank_line: Optional[LineString] = None,
    ) -> SectionData:
        if not sections:
        ...
        if target_measure <= float(ordered_sections[0].centerline_measure):
            first = ordered_sections[0]
            second = ordered_sections[1] if len(ordered_sections) > 1 else first
        elif target_measure >= float(ordered_sections[-1].centerline_measure):
            first = ordered_sections[-2] if len(ordered_sections) > 1 else ordered_sections[-1]
            second = ordered_sections[-1]
        else:
            first = ordered_sections[0]
            second = ordered_sections[-1]
            for idx in range(1, len(ordered_sections)):
                candidate_first = ordered_sections[idx - 1]
                candidate_second = ordered_sections[idx]
                if (
                    float(candidate_first.centerline_measure)
                    <= target_measure
                    <= float(candidate_second.centerline_measure)
                ):
        ...
        measure_span = (
            float(second.centerline_measure) - float(first.centerline_measure)
        )
        if abs(measure_span) <= 1e-9:
            fraction = 0.0
        else:
            fraction = (
                target_measure - float(first.centerline_measure)
            ) / measure_span
            fraction = max(0.0, min(1.0, fraction))

        def interp(first_value: float, second_value: float) -> float:
            return float(first_value) + fraction * (
                float(second_value) - float(first_value)
            )

        left_bank_measure = interp(
            first.left_bank_measure,
            second.left_bank_measure,
        )
        right_bank_measure = interp(
            first.right_bank_measure,
            second.right_bank_measure,
        )
        if left_bank_line is not None and left_bank_line.length > 0.0:
            left_point = left_bank_line.interpolate(
                max(0.0, min(float(left_bank_line.length), left_bank_measure))
            )
        else:
            left_point = Point(
                first.left_bank_point.x
                + fraction * (second.left_bank_point.x - first.left_bank_point.x),
                first.left_bank_point.y
                + fraction * (second.left_bank_point.y - first.left_bank_point.y),
            )
        if right_bank_line is not None and right_bank_line.length > 0.0:
            right_point = right_bank_line.interpolate(
                max(0.0, min(float(right_bank_line.length), right_bank_measure))
            )
        else:
            right_point = Point(
                first.right_bank_point.x
                + fraction * (second.right_bank_point.x - first.right_bank_point.x),
                first.right_bank_point.y
                + fraction * (second.right_bank_point.y - first.right_bank_point.y),
            )
        face_width = max(float(left_point.distance(right_point)), 1e-6)
        left_elevation = interp(
            HECRAS._elevation_at_station(first, first.left_bank_station),
            HECRAS._elevation_at_station(second, second.left_bank_station),
        )
        right_elevation = interp(
            HECRAS._elevation_at_station(first, first.right_bank_station),
            HECRAS._elevation_at_station(second, second.right_bank_station),
        )
        station_elevation = pd.DataFrame(
            [
                (0.0, left_elevation),
                (face_width, right_elevation),
            ],
        ...
        river_station = float(
            HECRAS._round_half_up(
                float(sections[-1].centerline_measure) - target_measure
            )
        )
        return replace(
            fallback_section,
            ...
            right_bank_station=face_width,
            left_bank_point=left_point,
            right_bank_point=right_point,
            channel_point=channel_point,
            left_bank_measure=left_bank_measure,
            right_bank_measure=right_bank_measure,
            centerline_measure=target_measure,
    ...
    def _build_section_inputs_from_selected_bank_lines(
        ...
        self._repair_terminal_bank_station_collapse(section_inputs)
    ...
    @staticmethod
    def _repair_terminal_bank_station_collapse(
        section_inputs: list[dict[str, Any]],
        min_neighbor_width_ratio: float = 0.75,
        station_tol: float = 1e-6,
    ) -> None:
        """Extend collapsed terminal bank stations from the adjacent section.

        Some supplied bank polylines taper or stop short at a reach end. In that
        case nearest-point projection can place both terminal bank stations close
        together, making the bank line visibly stop before the river reach end in
        HEC-RAS. The interior sections are left untouched; only an endpoint whose
        bank width is much smaller than its adjacent section is adjusted.
        """
        if len(section_inputs) < 2:
            return

        def bank_width(section: dict[str, Any]) -> float:
            return float(
                section["right_bank_entry"]["station"]
                - section["left_bank_entry"]["station"]
            )

        def channel_station(section: dict[str, Any]) -> float:
            return HECRAS._project_distance(
                section["line"],
                section["channel_point"],
            )

        def repair_endpoint(endpoint_idx: int, neighbor_idx: int) -> None:
            endpoint = section_inputs[endpoint_idx]
            neighbor = section_inputs[neighbor_idx]
            endpoint_width = bank_width(endpoint)
            neighbor_width = bank_width(neighbor)
            if neighbor_width <= station_tol:
                return
            if endpoint_width >= neighbor_width * float(min_neighbor_width_ratio):
                return

            neighbor_channel_station = channel_station(neighbor)
            endpoint_channel_station = channel_station(endpoint)
            left_offset = (
                float(neighbor["left_bank_entry"]["station"])
                - neighbor_channel_station
            )
            right_offset = (
                float(neighbor["right_bank_entry"]["station"])
                - neighbor_channel_station
            )
            ...
            left_station = max(
                0.0,
                min(float(line.length), endpoint_channel_station + left_offset),
            )
            right_station = max(
                0.0,
                min(float(line.length), endpoint_channel_station + right_offset),
            )
            if right_station - left_station <= endpoint_width + station_tol:
                return

            endpoint["hdf_left_bank_station"] = float(
                endpoint["left_bank_entry"]["station"]
            )
            endpoint["hdf_right_bank_station"] = float(
                endpoint["right_bank_entry"]["station"]
            )
            left_point = line.interpolate(left_station)
            right_point = line.interpolate(right_station)
            endpoint["left_bank_entry"] = {
                **endpoint["left_bank_entry"],
                "point": Point(left_point),
                "station": float(left_station),
            }
            endpoint["right_bank_entry"] = {
                **endpoint["right_bank_entry"],
                "point": Point(right_point),
                "station": float(right_station),
            }
            endpoint["terminal_bank_station_repair"] = True
            endpoint["channel_point"] = Point(
                (left_point.x + right_point.x) / 2.0,
                (left_point.y + right_point.y) / 2.0,
            )
            logger.info(
                "Extended terminal bank stations at source station %s "
                "from %.3f m to %.3f m using adjacent section offsets.",
                endpoint["source_station"],
                endpoint_width,
                right_station - left_station,
            )

        repair_endpoint(0, 1)
        repair_endpoint(len(section_inputs) - 1, len(section_inputs) - 2)
    ...
    def _finalize_sections(
        ...
        for section in filtered_inputs:
            ...
            has_terminal_bank_repair = bool(
                section.get("terminal_bank_station_repair", False)
            )
    ...
    def _build_centerline_geometry_simple_distance(
        ...
        for idx in range(len(filtered_inputs) - 1):
            ...
            segment_points = HECRAS._repair_centerline_segment_if_needed(
                current=current,
    ...
    @staticmethod
    def _repair_centerline_segment_if_needed(
        ...
        if len(proposed_points) < 2:
            return proposed_points[1:]

        corridor = HECRAS._build_local_channel_corridor(
            current=current,
            next_section=next_section,
            left_bank_line=left_bank_line,
            right_bank_line=right_bank_line,
        ...
        proposed_line = LineString(proposed_points)
        left_segment = HECRAS._oriented_bank_substring(
            left_bank_line,
            float(current["left_bank_measure"]),
            float(next_section["left_bank_measure"]),
        )
        right_segment = HECRAS._oriented_bank_substring(
            right_bank_line,
            float(current["right_bank_measure"]),
            float(next_section["right_bank_measure"]),
        )
        if HECRAS._centerline_segment_is_valid(
        ...
        midpoint_repair = HECRAS._build_nearest_bank_midpoint_segment(
            proposed_points=proposed_points,
            left_segment=left_segment,
            right_segment=right_segment,
            corridor=corridor,
        )
        if midpoint_repair is not None and HECRAS._centerline_segment_is_valid(
            proposed_line=LineString(midpoint_repair),
            corridor=corridor,
            left_segment=left_segment,
            right_segment=right_segment,
        ):
            return midpoint_repair[1:]

        repaired = HECRAS._build_voronoi_centerline_segment(
            start=Point(proposed_points[0]),
            end=Point(proposed_points[-1]),
            corridor=corridor,
            fallback_points=proposed_points,
            samples_per_segment=samples_per_segment,
        )
        if repaired is None:
            logger.warning(
                "Could not repair centerline segment between source stations %s "
                "and %s; keeping interpolated centerline.",
                current.get("source_station"),
                next_section.get("source_station"),
            )
            return proposed_points[1:]
        return repaired[1:]

    @staticmethod
    def _build_local_channel_corridor(
        current: dict[str, Any],
        next_section: dict[str, Any],
        left_bank_line: LineString,
        right_bank_line: LineString,
        ...
        left_coords = [(float(x), float(y)) for x, y in left_segment.coords]
        right_coords = [(float(x), float(y)) for x, y in right_segment.coords]
        ...
        corridor = Polygon(coords)
        if not corridor.is_valid:
            corridor = corridor.buffer(0)
        if corridor.is_empty:
            return None
        if corridor.geom_type == "MultiPolygon":
            current_point = Point(current["channel_point"])
            next_point = Point(next_section["channel_point"])
            candidates = [
                geom
                for geom in corridor.geoms
                if geom.buffer(1e-6).covers(current_point)
                or geom.buffer(1e-6).covers(next_point)
            ]
            corridor = max(candidates or corridor.geoms, key=lambda geom: geom.area)
        if corridor.geom_type != "Polygon" or corridor.area <= 0.0:
            return None
        return corridor

    @staticmethod
    def _build_nearest_bank_midpoint_segment(
        proposed_points: list[tuple[float, float]],
        left_segment: LineString,
        right_segment: LineString,
        corridor: Polygon,
    ) -> Optional[list[tuple[float, float]]]:
        if len(proposed_points) < 2:
            return None

        corridor_buffer = corridor.buffer(1e-6)
        repaired: list[tuple[float, float]] = [proposed_points[0]]
        for coord in proposed_points[1:-1]:
            point = Point(coord)
            _, left_point = nearest_points(point, left_segment)
            _, right_point = nearest_points(point, right_segment)
            midpoint = Point(
                0.5 * (float(left_point.x) + float(right_point.x)),
                0.5 * (float(left_point.y) + float(right_point.y)),
            )
            if not corridor_buffer.covers(midpoint):
                midpoint = point
            repaired.append((float(midpoint.x), float(midpoint.y)))
        repaired.append(proposed_points[-1])

        deduped: list[tuple[float, float]] = []
        for coord in repaired:
            if deduped and HECRAS._points_are_close(deduped[-1], coord):
                continue
            deduped.append(coord)
        return deduped if len(deduped) >= 2 else None

    @staticmethod
    def _oriented_bank_substring(
        bank_line: LineString,
        start_measure: float,
        end_measure: float,
    ) -> LineString:
        segment = HECRAS._substring_line(
            bank_line,
            min(float(start_measure), float(end_measure)),
            max(float(start_measure), float(end_measure)),
        )
        if float(start_measure) > float(end_measure):
            segment = LineString(list(segment.coords)[::-1])
        return segment

    @staticmethod
    def _centerline_segment_is_valid(
        proposed_line: LineString,
        corridor: Polygon,
        left_segment: LineString,
        right_segment: LineString,
        tolerance: float = 1e-6,
    ) -> bool:
        if not corridor.buffer(tolerance).covers(proposed_line):
            return False
        interior = proposed_line
        if interior.length > 2.0 * tolerance:
            interior = substring(proposed_line, tolerance, proposed_line.length - tolerance)
        if not interior.intersection(left_segment).is_empty:
            return False
        if not interior.intersection(right_segment).is_empty:
            return False
        return True

    @staticmethod
    def _build_voronoi_centerline_segment(
        start: Point,
        end: Point,
        corridor: Polygon,
        ...
        try:
            ...
            logger.warning("SciPy Voronoi unavailable for centerline repair: %s", exc)
            return None

        boundary_points = HECRAS._sample_polygon_boundary_points(
            corridor,
            sample_count=max(80, min(int(samples_per_segment), 300)),
        )
        if len(boundary_points) < 4:
            return None
        try:
            voronoi = Voronoi(np.asarray(boundary_points, dtype=float))
        except Exception as exc:
            logger.warning("Could not build Voronoi centerline repair graph: %s", exc)
            return None
        ...
        node_index: dict[tuple[float, float], int] = {
            HECRAS._node_key(nodes[0]): 0,
            HECRAS._node_key(nodes[1]): 1,
        }
        graph: dict[int, list[tuple[int, float]]] = {0: [], 1: []}
        corridor_buffer = corridor.buffer(1e-6)

        def add_node(coord: tuple[float, float]) -> int:
            key = HECRAS._node_key(coord)
        ...
        def add_edge(first: int, second: int) -> None:
            if first == second:
                return
            line = LineString([nodes[first], nodes[second]])
            if line.length <= 1e-9 or not corridor_buffer.covers(line):
                return
        ...
        for ridge_vertices in voronoi.ridge_vertices:
            if len(ridge_vertices) != 2 or min(ridge_vertices) < 0:
                continue
            first_coord = tuple(voronoi.vertices[ridge_vertices[0]])
            second_coord = tuple(voronoi.vertices[ridge_vertices[1]])
            if not corridor_buffer.covers(Point(first_coord)):
                continue
            if not corridor_buffer.covers(Point(second_coord)):
                continue
            add_edge(add_node(first_coord), add_node(second_coord))

        candidate_ids = [
            idx
            for idx, coord in enumerate(nodes[2:], start=2)
            if corridor_buffer.covers(Point(coord))
        ]
        ...
        for endpoint_id in (0, 1):
            nearest = sorted(
                candidate_ids,
                key=lambda idx: Point(nodes[endpoint_id]).distance(Point(nodes[idx])),
            )[:12]
            for candidate_id in nearest:
                add_edge(endpoint_id, candidate_id)
        direct_line = LineString([nodes[0], nodes[1]])
        if corridor_buffer.covers(direct_line):
            add_edge(0, 1)

        path = HECRAS._shortest_path(graph, start_id=0, end_id=1)
        if path is None or len(path) < 2:
            return None
        repaired = [nodes[idx] for idx in path]
        return HECRAS._densify_centerline_segment(
            repaired,
            target_count=max(2, len(fallback_points)),
        )
    ...
    def _sample_polygon_boundary_points(
        ...
        sample_count = max(int(sample_count), 8)
        distances = np.linspace(0.0, float(exterior.length), sample_count, endpoint=False)
        points = [
            exterior.interpolate(float(distance))
            for distance in distances
        ]
        return [(float(point.x), float(point.y)) for point in points]

    @staticmethod
    def _node_key(coord: tuple[float, float], decimals: int = 6) -> tuple[float, float]:
    ...
    def _shortest_path(
        ...
        while queue:
            ...
            for next_id, edge_distance in graph.get(node_id, []):
    ...
    @staticmethod
    def _densify_centerline_segment(
        points: list[tuple[float, float]],
        target_count: int,
    ) -> list[tuple[float, float]]:
        ...
        distances = np.linspace(0.0, float(line.length), target_count)
        sampled = [line.interpolate(float(distance)) for distance in distances]
        densified: list[tuple[float, float]] = []
        for point in sampled:
            coord = (float(point.x), float(point.y))
            if densified and HECRAS._points_are_close(densified[-1], coord):
                continue
            densified.append(coord)
        return densified
    ...
    def _compute_htab_parameters(section: SectionData) -> tuple[str, str, int]:
        min_elev = HECRAS._minimum_elevation_between_banks(section)
        max_elev = float(section.station_elevation["Elevation"].max())
        starting_elev = min_elev + 0.001
        table_count = 20
    ...
    @staticmethod
    def _estimate_terminal_friction_slopes(
        sections: list[SectionData],
    ) -> dict[str, float]:
        if len(sections) < 2:
            return {"upstream": 0.001, "downstream": 0.001}

        upstream_slope = HECRAS._estimate_pair_friction_slope(
            upstream_section=sections[-2],
            downstream_section=sections[-1],
            river_length=sections[-2].channel_reach_length,
        )
        downstream_slope = HECRAS._estimate_pair_friction_slope(
            upstream_section=sections[0],
            downstream_section=sections[1],
            river_length=sections[0].channel_reach_length,
        )
        upstream_terminal_min = HECRAS._minimum_elevation_between_banks(sections[-1])
        downstream_terminal_min = HECRAS._minimum_elevation_between_banks(sections[0])
        if downstream_terminal_min > upstream_terminal_min:
            upstream_slope, downstream_slope = downstream_slope, upstream_slope
        return {"upstream": upstream_slope, "downstream": downstream_slope}

    @staticmethod
    def _estimate_pair_friction_slope(
        upstream_section: SectionData,
        downstream_section: SectionData,
        river_length: float,
    ) -> float:
        if float(river_length) <= 1e-6:
            return 0.001
        upstream_min = HECRAS._minimum_elevation_between_banks(upstream_section)
        downstream_min = HECRAS._minimum_elevation_between_banks(downstream_section)
        slope = abs(float(upstream_min - downstream_min)) / float(river_length)
        return max(slope, 1e-4)
    ...
    def _write_steady_flow_file(
        ...
        upstream_friction_slope: Optional[float] = None,
        downstream_friction_slope: Optional[float] = None,
        flow_profiles: Optional[list[tuple[str, float]]] = None,
    ) -> None:
        upstream_slope = (
            float(friction_slope)
            if upstream_friction_slope is None
            else float(upstream_friction_slope)
        )
        downstream_slope = (
            float(friction_slope)
            if downstream_friction_slope is None
            else float(downstream_friction_slope)
        )
        profiles = (
        ...
        content = [
            ...
            "Up Type= 3 \n",
            f"Up Slope={HECRAS._format_number(upstream_slope, decimals=5)}\n",
            "Dn Type= 3 \n",
            f"Dn Slope={HECRAS._format_number(downstream_slope, decimals=5)}\n",
            "DSS Import StartDate=\n",
            "DSS Import StartTime=\n",
            "DSS Import EndDate=\n",
            "DSS Import EndTime=\n",
    ...
    def _write_junction_steady_flow_file(
        ...
        for reach_context, flow_key in zip(reaches, reach_flow_keys):
            ...
            if flow_key == "lower":
                downstream_slope = reach_context.get(
                    "friction_slopes",
                    {"downstream": reach_context["friction_slope"]},
                )["downstream"]
                content.extend(
                    [
                        "Up Type= 0 \n",
                        "Dn Type= 3 \n",
                        "Dn Slope="
                        f"{HECRAS._format_number(downstream_slope, decimals=5)}\n",
                    ]
                )
            else:
                upstream_slope = reach_context.get(
                    "friction_slopes",
                    {"upstream": reach_context["friction_slope"]},
                )["upstream"]
                content.extend(
                    [
                        "Up Type= 3 \n",
                        "Up Slope="
                        f"{HECRAS._format_number(upstream_slope, decimals=5)}\n",
                        "Dn Type= 0 \n",
                    ]
                )
        content.extend(
